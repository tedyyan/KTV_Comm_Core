...
void CMoiveControlApp::InitCard()
{
   ...
   if(g_splitter==NULL)
   {
	...
	if (g_mpeg_audio_decorder==NULL)
	{
		hr1 = CoCreateInstance(CLSID_FFDSHOW_AUDIO, NULL, CLSCTX_INPROC_SERVER, 
...
int CMoiveControlApp::PlayFile(CString szFile,bool encode,int movieType,int mode)
{
	...
	pigb->AddFilter(g_decoder, L"g_decoder");
	...
	//------------------------------------------------------------------------
	pigb->RemoveFilter(g_direct_sound_render);
	pigb->AddFilter(g_direct_sound_render,L"Direct sound render");
	//------------------------------------------------------------------------
	pigb->RemoveFilter(pDCDSPFilter);
	pigb->AddFilter(pDCDSPFilter,L"DCDSP Filter");
	//-------------------------------------------------------
	pigb->RemoveFilter(pAudioChannelMerger);
	pigb->AddFilter(pAudioChannelMerger,L"Audio Channel Merger");
	...
	try{
		...
		hr1 = CoCreateInstance(CLSID_AsyncSample_TEST, NULL, CLSCTX_INPROC_SERVER,
				IID_IBaseFilter, (LPVOID *) &g_source_render_encryptor);
		if (FAILED(hr1))
		{
			TRACE("\nerror in create myself g_source_render_encryptor");
			return -1;
		}
		TRACE("\npEncodeSourceFilter");

		if (g_source_render_encryptor ==NULL){
			TRACE("\nerror in get file g_source_render_encryptor ");
			return -1;
		}
		...
		if (g_source_render ==NULL){
			TRACE("\nerror in get file g_source_render ");
			return -1;
		}		
		TRACE("\npSourceFilter");
	

		pigb->RemoveFilter(g_source_render);
		pigb->AddFilter(g_source_render_encryptor,sourceFilterName);
		pigb->RemoveFilter(g_source_render);
		...
		if (encode){		
			...
			g_source_render_encryptor->QueryInterface(IID_IFileSourceFilter,(void**)&pBase);
			...
			sourcepin = GetPin(g_source_render_encryptor,PINDIR_OUTPUT);
		}else{
			g_source_render->QueryInterface(IID_IFileSourceFilter,(void**)&pBase);
			pBase->Load(wFile,NULL);
			sourcepin = GetPin(g_source_render,PINDIR_OUTPUT);
		}
		HELPER_RELEASE(pBase);	

		if (sourcepin==NULL){
			TRACE("\nCan't get source pin");
			return -1;
		}
		...
		if ((movieType==2)||(movieType==3)){
			...
			pigb->RemoveFilter(aviSplitter);
			pigb->AddFilter(aviSplitter, L"CLSID_RM_AVI_SPLITTER");
		
			pigb->RemoveFilter(layer3);
			pigb->AddFilter(layer3, L"CLSID_MPEG_LAYER_3_DECODER");
			
			splitterIn=GetPin(aviSplitter,PINDIR_INPUT);

			hr=pigb->ConnectDirect(sourcepin,splitterIn,NULL);
			if (FAILED(hr))
			{
				TRACE("\nCannot link source and splitter in AVI!!!");
				return -1;
			}

			splitterAout=FetchpPin(aviSplitter, PINDIR_OUTPUT,"A");
			splitterVout=FetchpPin(aviSplitter, PINDIR_OUTPUT,"V");
			
			
			decodeVin=FetchpPin(g_decoder, PINDIR_INPUT,"V");
			
			IPin* layer3in=GetPin(layer3, PINDIR_INPUT);
			hr=pigb->ConnectDirect(splitterAout,layer3in,NULL);
			if (FAILED(hr))
			{
				TRACE("Cannot splitter and layer3in in AVI!!!");
				return -1;
			}
			hr=pigb->ConnectDirect(splitterVout,decodeVin,NULL);
			if (FAILED(hr))
			{
				TRACE("Cannot splitterVout and decodeVin in AVI!!!");
				return -1;
			}
		...
		}else  //(movieType==2)||(movieType==3)
		{   
			
			//----------------------------------------
			pigb->RemoveFilter(g_splitter);
			pigb->AddFilter(g_splitter, L"CLSID_MPEG2_Splitter");
			//----------------------------------------
			hr1 = pigb->RemoveFilter(g_mpeg_audio_decorder);
			hr1 = pigb->AddFilter(g_mpeg_audio_decorder,L"decoder");			
			//----------------------------------------


			splitterIn=FindPinByDirection(g_splitter,PINDIR_INPUT);
			...
			splitterAout=FetchpPin(g_splitter, PINDIR_OUTPUT,"A");
			splitterVout=FetchpPin(g_splitter, PINDIR_OUTPUT,"V");
			...
			hrd=pigb->ConnectDirect(splitterVout,decodeVin,NULL);
			if (FAILED(hrd)){
				TRACE("\nDirectly link with pindecodeV failed\n");
				return -1;
			}
			hrd=pigb->ConnectDirect(splitterAout,mpeg1audioInput,NULL);
			if (FAILED(hrd)){
