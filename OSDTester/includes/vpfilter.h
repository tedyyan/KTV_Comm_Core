...
typedef NTSTATUS(*HOOK_VPF)(
                IN PVPFILTER_CALLBACK VPFilterCallBack,
                IN PVOID              Context
            );

typedef NTSTATUS(*UNHOOK_VPF)(
            IN PVPFILTER_CALLBACK VPFilterCallBack
            );

typedef struct _VPF_HOOKS{
	HOOK_VPF	fHookVideoFilter;
	UNHOOK_VPF	fUnHookVideoFilter;
}VPF_HOOKS,*PVPF_HOOKS;

#define	VPFILTER_NAME L"\\Device\\vpfilter"

#define IOCTL_VPF_GET_FUNC_POINTERS   CTL_CODE(0x00009123U,         \
                                               0x9123,				\
                                               METHOD_BUFFERED,		\
                                               FILE_ANY_ACCESS)
